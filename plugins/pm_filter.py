...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        watch_btn = InlineKeyboardButton("🗑️ Remove Watchlist", callback_data=f"del_watch#{file_id}") if str(file_id) in user_watchlist else InlineKeyboardButton("𝐆𝐫𝐨𝐮𝐩", url=SUPPORT_LINK)
        fav_btn = InlineKeyboardButton("💔 Remove Favorites", callback_data=f"del_fav#{file_id}") if str(file_id) in user_favorites else InlineKeyboardButton("𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url=UPDATES_LINK)
        btn = [[
            ...
            InlineKeyboardButton('✖️ 𝐂𝐥𝐨𝐬𝐞', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, s, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b><i>Here i found for your search {search}...</b></i>"
    CAP[key] = cap
    auto_del_time = settings.get("auto_delete_time", DELETE_TIME)
    del_msg = f"\n\n<b><blockquote>©𝐓𝐞𝐚𝐦 𝐔𝐫𝐯𝐚𝐬𝐡𝐢 𝐓𝐡𝐞𝐚𝐭𝐞𝐫𝐬</blockquote></b>" if settings["auto_delete"] else ''
    if imdb and imdb.get('poster'):
        await s.delete()
        try:
            k = await message.reply_text(cap[:1024] + files_link + del_msg, reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML, reply_parameters=ReplyParameters(message_id=message.id))
