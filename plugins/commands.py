...
async def start(client, message):
    ...
    if mc.startswith('all'):
        ...
        for file in files:
            ...
            watch_btn = InlineKeyboardButton("🗑️ Remove Watchlist", callback_data=f"del_watch#{f_id_str}") if f_id_str in user_watchlist else InlineKeyboardButton("𝐆𝐫𝐨𝐮𝐩", url=SUPPORT_LINK)
            fav_btn = InlineKeyboardButton("💔 Remove Favorites", callback_data=f"del_fav#{f_id_str}") if f_id_str in user_favorites else InlineKeyboardButton("𝐂𝐡𝐚𝐧𝐧𝐞𝐥", url=UPDATES_LINK)
